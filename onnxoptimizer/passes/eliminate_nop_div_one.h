...
struct EliminateNopDivOne final : public PredicateBasedPass {
  explicit EliminateNopDivOne()
      : PredicateBasedPass(PassType::Nop, PassEfficiency::Complete,
                           PassOptimizationType::Compute) {}

  std::string getPassName() const override {
    return "eliminate_nop_div_one";
  }
...
#undef CASE_BRANCH_CONTENT
  ...
  bool patternMatchPredicate(Node* node) override {
    return node->kind() == kDiv && node->inputs()[1]->node()->kind() == kParam;
  }
  ...
  bool runTransform(Node* node, Graph& graph,
    ...
    if (isConstantTensor(graph, b_name) && isTensorOfOne(*b_tensor)) {
      replacing_success =
