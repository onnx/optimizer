...
struct FusePadIntoAvgPool final : public PredicateBasedPass {
  explicit FusePadIntoAvgPool()
      : PredicateBasedPass(PassType::Fuse, PassEfficiency::Complete,
                           PassOptimizationType::Compute) {}
  std::string getPassName() const override {
    return "fuse_pad_into_avgpool";
  }
  ...
  bool runTransform(Node* n, Graph& graph,
    ...
    int avgpool_pads_size = pads_size - 4;
    std::vector<int64_t> avg_pads(avgpool_pads_size, 0);
    // Fuse into existing padding, if available
    if (avgpool->hasAttribute(kpads)) {
      avg_pads = avgpool->is(kpads);
    }

    for (int i = 2, j = 0; i < pads_size / 2; ++i, ++j) {
      avg_pads[j] += pads[i];
      avg_pads[avgpool_pads_size / 2 + j] += pads[pads_size / 2 + i];
    }

    avgpool->is_(kpads, std::move(avg_pads));
    avgpool->replaceInput(0, pad->inputs()[0]);
    pad->destroy();
