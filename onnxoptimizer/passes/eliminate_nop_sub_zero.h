...
struct EliminateNopSubZero final : public PredicateBasedPass {
  explicit EliminateNopSubZero()
      : PredicateBasedPass(PassType::Nop, PassEfficiency::Complete,
                           PassOptimizationType::Compute) {}

  std::string getPassName() const override {
    return "eliminate_nop_sub_zero";
  }
  ...
  template <typename T>
  bool isAllValue(const std::vector<T>& data, const T& val = {0}) {
    return std::all_of(data.cbegin(), data.cend(),
                       [&val](const T& v) { return v == val; });
  }

  bool isTensorOfZero(const Tensor& tensor) {
    int elem_type = tensor.elem_type();

#define CASE_BRANCH_CONTENT(pb_dtype, cpp_dtype, zero)                 \
  case ONNX_NAMESPACE::TensorProto_DataType_##pb_dtype: {              \
    const std::vector<cpp_dtype> data = ParseData<cpp_dtype>(&tensor); \
    return isAllValue<cpp_dtype>(data, zero);                          \
  }

    switch (elem_type) {
      CASE_BRANCH_CONTENT(FLOAT, float, float{0})
      CASE_BRANCH_CONTENT(INT32, int32_t, int32_t{0})
      CASE_BRANCH_CONTENT(INT64, int64_t, int64_t{0})
      CASE_BRANCH_CONTENT(DOUBLE, double, double{0})
      CASE_BRANCH_CONTENT(FLOAT16, int32_t, 0)
      CASE_BRANCH_CONTENT(UINT8, int32_t, int32_t{0})
      CASE_BRANCH_CONTENT(INT8, int32_t, int32_t{0})
      CASE_BRANCH_CONTENT(UINT16, int32_t, int32_t{0})
      CASE_BRANCH_CONTENT(INT16, int32_t, int32_t{0})
      // TODO: support uint64
...
#undef CASE_BRANCH_CONTENT
  ...
  bool patternMatchPredicate(Node* node) override {
    return node->kind() == kSub && node->inputs()[1]->node()->kind() == kParam;
  }
  ...
  bool runTransform(Node* node, Graph& graph,
    ...
    if (isConstantTensor(graph, b_name) && isTensorOfZero(*b_tensor)) {
      replacing_success =
